...
{
    public partial class SectionD_SupplyLog : Form
    {
        public SectionD_SupplyLog()
        {
            InitializeComponent();
        }
        GetDataBase obj = new GetDataBase();
        private void SectionD_SupplyLog_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                dtgetsupplylog_dgv = obj.getdatabase("select staffname,supplyid,productid,itemsupplied,quantitysupplied," +
                    "unitsalesprice,amountsupplied,siv,entrydate from dsupplylog order by entrydate");
                dtgetsupplylog = obj.getdatabase("select * from dsupplylog order by entrydate");

                System.Data.DataTable dtgetsupplylog1 = new System.Data.DataTable();
                dtgetsupplylog1 = obj.getdatabase("select amountsupplied,profit from dsupplylog");
                double temp = 0;
                double temp1 = 0;
                int c = 0;
                ...
                dtgetsupplylog = obj.getdatabase("select * from dsupplylog order by date");
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dtgetsupplylog_dgv = obj.getdatabase("select staffname,supplyid,productid,itemsupplied,quantitysupplied," +
                    ...
                    {
                        dtgetsupplylog_dgv = obj.getdatabase("select staffname,supplyid,productid,itemsupplied,quantitysupplied," +
                     "unitsalesprice,amountsupplied,siv,entrydate from dsupplylog where  staffname='" + cbocashier.Text + "' And date='" + DateTimePicker1.Value.Date.ToShortDateString() + "'");
                        dgvsaleslog.DataSource = dtgetsupplylog_dgv;
                        ...
                        txtcustomer.Text = c.ToString();
                        dtgetsupplylog1 = obj.getdatabase("select amountsupplied,profit from dsupplylog where staffname='" + cbocashier.Text + "' And date='" + DateTimePicker1.Value.Date.ToShortDateString() + "'");
                        if (dtgetsupplylog1.Rows.Count > 0)
                        {
                            for (var k = 0; k < dtgetsupplylog1.Rows.Count; k++)
                            {
                    ...
                    else if (DateTimePicker1.Value.Date.ToShortDateString() != DateTimePicker2.Value.Date.ToShortDateString() && cbocashier.Text != "Daily Supply")
                    {
                        ...
                        dtgetsupplylog_dgv = obj.getdatabase("select staffname,supplyid,productid,itemsupplied,quantitysupplied," +
                     "unitsalesprice,amountsupplied,siv,entrydate from dsupplylog where date >='" + stdate + "' and date <='" + eddate + "'and  staffname='" + cbocashier.Text + "'");
                        if (dtgetsupplylog.Rows.Count > 0)
                        {
                            c = dtgetsupplylog.Rows.Count;
                        }
                        txtcustomer.Text = dtgetsupplylog_dgv.Rows.Count.ToString();
                        dgvsaleslog.DataSource = dtgetsupplylog_dgv;
                        dtgetsupplylog1 = obj.getdatabase("select amountsupplied,profit from dsupplylog where date >='" + stdate + "' and date <='" + eddate + "' and staffname='" + cbocashier.Text + "'");
                        if (dtgetsupplylog1.Rows.Count > 0)
                        {
                    ...
                    {
                        dtgetsupplylog_dgv = obj.getdatabase("select staffname,supplyid,productid,itemsupplied,quantitysupplied," +
                     "unitsalesprice,amountsupplied,siv,entrydate from dsupplylog where date='" + DateTimePicker1.Value.Date.ToShortDateString() + "'");
                        dgvsaleslog.DataSource = dtgetsupplylog_dgv;
                        ...
                        txtcustomer.Text = dtgetsupplylog_dgv.Rows.Count.ToString();
                        dtgetsupplylog1 = obj.getdatabase("select amountsupplied,profit from dsupplylog where date='" + DateTimePicker1.Value.Date.ToShortDateString() + "'");
                        if (dtgetsupplylog1.Rows.Count > 0)
                        {
                            for (var k = 0; k < dtgetsupplylog1.Rows.Count; k++)
                            {
                    ...
                    else if (cbocashier.Text == "Daily Supply" && DateTimePicker1.Value.Date.ToShortDateString().ToString() != DateTimePicker2.Value.Date.ToShortDateString().ToString())
                    {
                        ...
                        dtgetsupplylog_dgv = obj.getdatabase("select staffname,supplyid,productid,itemsupplied,quantitysupplied," +
                     "unitsalesprice,amountsupplied,siv,entrydate from dsupplylog where date >='" + stdate + "' and date <='" + eddate + "'");
                        if (dtgetsupplylog.Rows.Count > 0)
                        {
                            c = dtgetsupplylog.Rows.Count;
                        }
                        txtcustomer.Text = dtgetsupplylog_dgv.Rows.Count.ToString();
                        dgvsaleslog.DataSource = dtgetsupplylog_dgv;
                        dtgetsupplylog1 = obj.getdatabase("select amountsupplied,profit from dsupplylog where date >='" + stdate + "' and date <='" + eddate + "'");
                        if (dtgetsupplylog1.Rows.Count > 0)
                        {
                            for (var j = 0; j < dtgetsupplylog1.Rows.Count; j++)
                            {
